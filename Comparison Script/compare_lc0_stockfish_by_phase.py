...
results = []
for sf_entry, lc0_entry in zip(sf_data, lc0_data):
    assert sf_entry["fen"] == lc0_entry["fen"] and sf_entry["phase"] == lc0_entry["phase"], "FEN or phase mismatch!"

    move_match = sf_entry["best_move"] == lc0_entry["lc0_move"]
    eval_diff = sf_entry["eval_after_move"] - lc0_entry["stockfish_eval_on_lc0_move"]
...
#plot
ax = sns.histplot(filtered, bins=bins, kde=True, color="skyblue", edgecolor="black")
...
plt.title("Stockfish Evaluation Difference (SF - Lc0 Move)")
...
plt.tight_layout()
plt.savefig("phase_eval_summary_table_sf_lc0.png", dpi=300)
plt.close()