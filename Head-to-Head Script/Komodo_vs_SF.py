...
for game_num in range(num_games):
    ...
    with chess.engine.SimpleEngine.popen_uci(stockfish_path) as sf_engine, \
         chess.engine.SimpleEngine.popen_uci(komodo_path) as komodo_engine:
...
plt.bar(results.keys(), results.values(), color=["mediumseagreen", "orangered", "gray"])
plt.title("Komodo vs. Stockfish – Match Results")
plt.ylabel("Number of Games")
plt.grid(axis="y", linestyle="--", alpha=0.7)
plt.tight_layout()
plt.savefig("2.5s_komodo_vs_stockfish_match_results.png")
plt.show()
